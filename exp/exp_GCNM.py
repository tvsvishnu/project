...
class Exp_GCNM(Exp_GCNMbasic):
    ...
    def _build_model(self):
        ...
        elif self.model_name == 'DMSTGCN':
            model = model_dict[model_name](
                self.device,
                num_nodes = 207, dropout=0.3,
    ...
    def vali(self, vali_loader):
        ...
        for i, (batch_x, batch_dateTime, batch_y) in enumerate(vali_loader.get_iterator()):
            ...
            elif self.model_name == "GCNM" or self.model_name == "GCNMdynamic" or self.model_name == "DMSTGCN":
    ...
    def train(self):
        ...
        if self.mask_option == "random":
            stat_file = os.path.join(self.root_path, "random_missing", "rand/MissRatio_{:.2f}%.npz".format((1 - self.mask_ones_proportion) * 100))
        else:
            stat_file = os.path.join(self.root_path, "mix_missing",
                                     "mix/MissRatio_{:.2f}%.npz".format((1 - self.mask_ones_proportion) * 100))
        ...
        print(stat_file)
        stat_file = "Datasets/METR-LA/random_missing/rand/MissRatio_60.00%.npz"
        ...
        for epoch in range(self.train_epochs):
            ...
            for i, (batch_x, batch_dateTime, batch_y) in enumerate(train_loader.get_iterator()):
                ...
                if self.model_name == "gwnet":
                    ...
                    batch_x = batch_x.permute(0, 3, 1, 2)  # (B, D, 8, L)
                    batch_y = batch_y.permute(0, 2, 1)  # (B, L, D)
